  ...
  <main class="main-folders-container" v-if="!loading">
    ...
    <div v-if="!isSearching" :class="{ 'main-folder-principal': true, 'shrinked': showModalFileOption }">
      ...
      <div class="main-container-grid-a">
        <div v-for="(folder, index) in folders" :key="index" class="main-folders-block-a"
          :class="{ 'selected': selectedItem === folder }" @dblclick="openFolder(folder)"
          @click.right="selectFolder(folder)" @contextmenu.prevent="openModal('carpeta', folder)" draggable="true"
          @dragover.prevent @drop="handleDrop(folder)" @dragstart="handleDragStart(folder)">
          <svg class="folder-svg-img" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="100%" viewBox="0 0 512 512"
            enable-background="new 0 0 512 512" xml:space="preserve">
            <path fill="#FFFFFF" opacity="1.000000" stroke="none" d="M1.000003,406.000000 
      ...
      <div class="main-container-grid-b">
        <div v-for="(file, index) in files" :key="index" class="main-folders-block-b"
          :class="{ 'selected': selectedItem === file.path }" @contextmenu.prevent="openModal('archivo', file.path)"
          @click.right="selectFile(file.path)" draggable="true" @dragover.prevent
          @dragstart="handleDragStart(file.path)" @drop="handleDrop(file.path)">
